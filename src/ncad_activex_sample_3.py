...
if ncad_doc is not None:
    ...
    Рассмотрми далее некоторые сценарии работ со свойствами документа и стилями:
    """
    ...
    неких параметров, храняющихся отдельно от документа. Тогда можно будет всегда сопоставить данный документ с набором 
    ...
    альтернативу Susyem.Windows.Forms.OpenFileDialog из .NET API или IronPython и похожей процедуры из-пол Windows-api у 
    С++. 
    ...
    геометрии: GetPoint, GetPolylineб GetAngle. Специфика "геометрических" методов в том, что Пользователь их не 
    выбирает, а именно рисует, и как результат метода -- возвращается нарисованная вреемнна ягеометрия в виде чисел. 
    ...
    """
    Работа с базой данных чертежа (интерфейс OdaX.IAcadDatabase) сводится к получению анотативных стилей (при 
