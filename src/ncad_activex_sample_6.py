...
if ncad_doc is not None:
    ...
    замкнутую область (кроме замкнутых полилиний)
    ...
    def lw_pline_work():
        ...
        То есть практично создавать методом AddLightWeightPolyline() и не задаваться подобным вопросом в будущем.

        Обращаем также внимание, что список образующих точек VerticesList - это не список списков 2 координат, а
        именно единый список координат вида [x1, y1, x2, y2 .... xn, yn]. Также обращаем внимание, что это "плоская
        геометрия", то есть потребны 2 координаты - только X и Y
        """
