...
if ncad_doc is not None:
    ...
    сперва создается её нематериальное определение, а потом к нему добавляются контуры с заполнением. Рассмотрим далее
     эту механику на примере штриховки типа SOLID для прямоугольной области, а затем получим параметры этой штриховки:
     
     Первый параметр: PatternType это enum AcPatternType, причем он не может быть изменен после создания 
     ...
     штриховки от изменения родительский сущностей-границ
    ...
    у нас имеется метод InsertLoopAt(),который и добавляет (или заменяет). Альтернативный вариант, при формировании только 
    внешней границы можно воспользоваться методом AppendOuterLoop(). Мы же воспользуемся общим случаем InsertLoopAt и 
    сделаем 2 не пересекающих друг друга контура:
    """
